            ...
            <Reveal key={item.name} delay={(i % 3) * 0.12} y={40} margin="-80px">
              ...
              >
                ...
                <div className="relative h-56 overflow-hidden">
                  ...
                  <span className="absolute right-4 top-4 rounded-full border border-fg/15 bg-ink/40 px-3 py-1 font-mono text-xs text-gold backdrop-blur-md">
                    {item.price}
                  </span>
                </div>
                <div className="flex flex-1 flex-col p-6">
                  <h3 className="text-xl font-bold tracking-tight">{item.name}</h3>
                  <p className="mt-2 flex-1 text-sm leading-relaxed text-fg/50">
                    {item.desc}
                  </p>
                </div>
              </motion.article>
